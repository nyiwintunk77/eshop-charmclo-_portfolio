                                ...
                                <article class="card-body payment-article">
                                    <?php 
                                        try {
                                            ...
                                            $info_sql = "SELECT Sum(price*quantity) as totalPrice, cus.c_name FROM cart INNER JOIN item on cart.item_id = item.item_id INNER JOIN customers cus ON cart.c_code = cus.c_code WHERE cart.c_code = ? GROUP BY cus.c_name" ;
                                            $st3 = $pdo->prepare($info_sql);
